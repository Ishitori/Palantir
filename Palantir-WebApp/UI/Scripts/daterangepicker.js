...
!function ($) {
    ...
    var DateRangePicker = function (element, options, cb) {
        ...
        if (hasOptions) {
            ...
            if (typeof options.periods == 'object')
            ...
            if (typeof options.ranges == 'object') {
                ...
                for (var range in this.ranges) {
                    rangesNames.push(range);
                }

                for (var i = 0; i < rangesNames.length; i++) {
                    if (rangesNames.length == this.periods.length) {
                        list += '<li period=' + this.periods[i] + '>' + rangesNames[i] + '</li>';
                    } else {
                        
                            list += '<li>' + rangesNames[i] + '</li>';
                        
                    }
                }

                list += '<li period="other">' + this.locale.customRangeLabel + '</li>';
                list += '</ul>';
    ...
    DateRangePicker.prototype = {
        ...
        updateFromControl: function () {
            ...
            this.updateView();
            this.cb(this.startDate, this.endDate, this.period);
            this.updateCalendars();
        },
        ...
        move: function () {
            ...
            if (this.opens == 'left') {
                this.container.css({
                    top: this.element.offset().top + this.element.offsetHeight(),
        ...
        clickRange: function (e) {
            var label = e.target.innerHTML;
            var period = $(e.target).attr('period');
            if (label == this.locale.customRangeLabel) {
                this.container.find('.calendar').show();
                this.container.find('.btn-success').show();
                this.move();
            } else {
                var dates = this.ranges[label];
                ...
                this.container.find('.btn-success').hide();
                this.move();
                this.hide();
            }
        },
